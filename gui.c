...
void gui_draw_text(int column, int row, char *string, char *colors, int flags) {
  ...
  if (menu_draw_widget) {
    ...
    if (option_blender_font) {
      glEnable(GL_TEXTURE_2D); glBindTexture(GL_TEXTURE_2D, new_letters);
    } else {
      glEnable(GL_TEXTURE_2D); glBindTexture(GL_TEXTURE_2D, old_letters);
    };
    if (flags < 0) {
...
int gui_button(int x, int y, int w, char *string, int flags) {
  ...
  if (menu_draw_widget) {
    ...
    if (drawdown) {
      glEnable(GL_TEXTURE_2D); glBindTexture(GL_TEXTURE_2D, widgets);
      glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
      COLOR_TOUCH;
      glBegin(GL_QUADS);
      #define X3 (14 / 256.0)
...
void gui_texture(int column, int line, int width, int height, int texture, double ts, double tt, double tu, double tv) {
  ...
  glEnable(GL_TEXTURE_2D);
  glCallList(texture_list_base + texture);
...
void gui_draw_block (int block_type, int column, int line, int width, int height) {
  ...
  if (block_data[block_type].visible) {
    ...
    glBindTexture(GL_TEXTURE_2D, texture_data[block_data[block_type].index[BLOCK_SIDE_UP]].name);
    glBegin(GL_QUADS);
    ...
    glBindTexture(GL_TEXTURE_2D, texture_data[block_data[block_type].index[BLOCK_SIDE_FRONT]].name);
    glBegin(GL_QUADS);
    ...
    glBindTexture(GL_TEXTURE_2D, texture_data[block_data[block_type].index[BLOCK_SIDE_RIGHT]].name);
    glBegin(GL_QUADS);
